// src/realtime/likes-listener.ts
import supabase from '@/app/lib/supabase-client';

// リアルタイム変更イベントの型定義
interface LikeChange {
  event: 'INSERT' | 'UPDATE' | 'DELETE';
  data: any; // 将来的には `likes` テーブルの具体的な型に置き換え可能
}

/**
 * いいねの変更をリアルタイムで購読する関数
 * @param postId 投稿ID（またはいいね対象のID）
 * @param callback 変更イベントを受け取るコールバック関数
 * @returns 購読を解除する関数
 */
export const subscribeToLikes = (
  postId: string,
  callback: (change: LikeChange) => void,
) => {
  // ユニークなチャンネル名を生成
  const channel = supabase
    .channel(`likes_${postId}`)
    .on(
      'postgres_changes',
      {
        event: '*', // 挿入、更新、削除のすべてのイベントをリッスン
        schema: 'public',
        table: 'likes',
        filter: `post_id=eq.${postId}`, // 指定した投稿IDにフィルタリング
      },
      (payload) => {
        // イベントタイプに応じてコールバックに適切なデータを渡す
        if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
          callback({ event: payload.eventType, data: payload.new });
        } else if (payload.eventType === 'DELETE') {
          callback({ event: 'DELETE', data: payload.old });
        }
      },
    )
    .subscribe();

  // クリーンアップ関数を返す（購読解除用）
  return () => {
    supabase.removeChannel(channel);
  };
};